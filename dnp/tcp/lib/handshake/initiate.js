...
async function initiate(node, ctx, socket, options) {
  ...
  const flags = (0, _common.ourFlags)(node);
  ...
  try {
    return new Promise((resolve, reject) => {
      ...
      function onConnect() {
        socket.off('close', cleanAndReject);
        socket.off('error', cleanAndReject);
        initiateHandshakeV6().then(([nodeInformation, chunk]) => resolve([nodeInformation, chunk]), reject);
      }
  ...
  async function initiateHandshakeV6() {
    ...
    socket.write(message, () => log(ctx, 'initiateHandshake(%o) : written', message));
  ...
  async function awaitStatusResponse() {
    ...
    log(ctx, 'awaitStatusResponse(status: %o)', chunk.subarray(3, 3 + length - 1));
  ...
  async function continueHandshake(chunk) {
    ...
    if (chunk.length >= 21) {
      ...
      if (chunk.length >= 21 + nameLength) {
        ...
        const answer = (0, _common.computeChallenge)(challenge, outCookie);
        const ourChallenge = await sendChallengeResponse(answer);
        return receiveChallengeAcknowledgement(ourChallenge, chunk.subarray(21 + nameLength));
      } else {
        const nextChunk = await (0, _common.waitForChunk)(ctx, socket);
  ...
  function validateAnswer(challenge, answer, cookie) {
    const ourAnswer = (0, _common.computeChallenge)(challenge, cookie);
  ...
  async function sendChallengeResponse(answer) {
    ...
    return (0, _common.computeChallenge)(challenge, outCookie);
  }
  ...
  async function receiveChallengeAcknowledgement(challenge, chunk) {
    if (chunk.length >= 2) {
      ...
      if (chunk.length >= length + 2) {
        ...
        const nextChunk = await (0, _common.waitForChunk)(ctx, socket);
        return receiveChallengeAcknowledgement(challenge, Buffer.concat([chunk, nextChunk]));
      }
    } else {
      const nextChunk = await (0, _common.waitForChunk)(ctx, socket);
      return receiveChallengeAcknowledgement(challenge, Buffer.concat([chunk, nextChunk]));
