...
export default async function receive(node, ctx, socket, options) {
    ...
    const sum = await challenge(
        ctx,
        socket,
        typeof node.name === 'symbol' ? Symbol.keyFor(node.name) : node.name,
        ourFlags(node),
        outCookie
    );
    ...
    function receiveV6Handshake(chunk) {
        ...
        for (let flag in Flags) {
            log(
                'receiveV6Handshake(flag: %o, bit: 0x%s, set: %o)',
                flag,
                Flags[flag].toString(16),
                (Flags[flag] & flags) > 0n
            );
        ...
        log(
            'receiveV6Handshake(flags: 0x%s, flagArray: %O)',
            flags.toString(16),
            flagArray
        );
    ...
    async function _answerIncomingChallenge(challenge) {
        ...
        const answer = computeChallenge(challenge, inCookie);
    ...
    async function issueV5Challenge(ctx, socket, name, flags, inCookie) {
        ...
        socket.write(buffer, () =>
            log('issueV5Challenge(%o) : drained', buffer)
        );

        return computeChallenge(challenge.readUInt32BE(0), inCookie);
    }
    async function issueV6Challenge(ctx, socket, name, flags, inCookie) {
        const challenge = crypto.randomBytes(4);
        log(
            'issueV6Challenge(name: %o, challenge: %o, flags: %s)',
            name,
            challenge,
            flags.toString(16)
        );
