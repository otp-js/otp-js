...
const log = (0, _debug.default)('otpjs:transports:ertf:parser:term');

function make(env, options) {
  const parsers = new Map([[97, parseSmallInt], [98, parseInt], [99, parseFloatString], [88, parsePid], [104, parseTuple], [105, parseLargeTuple], [116, parseMap], [106, parseNil], [107, parseString], [108, parseList], [109, parseBinary], [110, parseSmallBigNum], [111, parseLargeBigNum], [90, parseRef], [70, parseBetterFloat], [118, parseUTF8Atom], [119, parseSmallUTF8Atom]]);
  return deserialize;
  ...
  function deserialize(buff) {
    ...
    log('parseTerm(buff: %o, startTerm: %o, parser: %o)', buff, startTerm, parser);
  ...
  function parsePid(buff) {
    ...
    const term = new _types.Pid(nodeId, id, serial, creation);
  ...
  function parseTuple(buff) {
    ...
    const term = _types.tuple.create(arity);
  ...
  function parseNil(buff) {
    return [_types.list.nil, buff];
  }

  function parseString(buff) {
    const length = buff.readUInt16BE(0);
    return [(0, _types.l)(...buff.subarray(2, 2 + length)), buff.subarray(2 + length)];
  }
  ...
  function parseList(buff) {
    ...
    list.tail = tail;
    return [(0, _types.il)(...list, tail), nextBuff];
  }
}