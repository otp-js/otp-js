...
describe('List', function () {
    ...
    describe('includes', function () {
        it('scans the list for value, returning true if found, false if not', function () {
            ...
            expect(list.includes(4)).to.equal(false);
            expect(improper.includes(2)).to.equal(true);
    ...
    describe('find', function () {
        it('finds and returns the first value for which predicate is true', function () {
            ...
            expect(list.find((value) => value === 4)).to.equal(undefined);
            expect(improper.find((value) => value === 2)).to.equal(2);
    ...
    describe('nth', function () {
        it('returns the Nth item of the list', function () {
            ...
            expect(list.nth(2)).to.equal(3);
            expect(list.nth(3)).to.equal(4);
            expect(list.nth(4)).to.equal(5);
    ...
    describe('ImproperList', function () {
        ...
        it('cannot be constructed without a tail', function () {
            ...
            expect(l.isList(valid.tail)).to.equal(false);
        ...
        it('includes an isList function', function () {
            ...
            expect(il.isList(improper.tail)).to.equal(false);
    ...
    describe('inspection', function () {
        ...
        it('returns a shortened form if size is greater than maxArrayLength', function () {
            ...
            expect(util.inspect(l(1, 2, 3), { maxArrayLength: 2 })).to.equal(
                '[ 1, 2 ... 1 more items ]'
            );
            expect(util.inspect(il(1, 2, 3), { maxArrayLength: 2 })).to.equal(
                '[ 1, 2 | 3 ]'
            );
        });
    ...
    describe('toString', function () {
        ...
        describe('with an object child containing a toString method', function () {
            it('calls the toString method on the object', function () {
                ...
                expect(list.toString()).to.equal('[ 1, 2, object ]');
            });
        });
    ...
    describe('map', function () {
        ...
        it('runs the given function over every element of the list', async function () {
            ...
            await expect(list.map(fn)).to.eventually.be.ok;
            expect(fn).to.have.callCount(elements.length);
        ...
        it('creates a new list of the return values for each run', async function () {
            ...
            const list = l(...elements);
            const fn = sinon.spy(transform);

            const promise = list.map(fn);
            await expect(promise).to.eventually.be.ok;
            expect(fn).to.have.callCount(elements.length);
    ...
    describe('filter', function () {
        ...
        it('creates a list of elements for which the function returned true', async function () {
            ...
            for (let index = 0; index < elements.length; index++) {
                expect(fn.getCall(index).args[0]).to.equal(elements[index]);
                expect(fn.getCall(index).returnValue).to.equal(results[index]);

                if (results[index]) {
                    const head = car(it);
                    expect(head).to.equal(elements[index]);
                    it = cdr(it);
                }
            }
        });
        ...
        describe('on nil', function () {
            ...
            it('splits the list at the point where the predicate returns true', function () {
                ...
                for (let index = 0; index < expectedA.length; index++) {
                    ...
                    expect(headA).to.equal(expectedA[index]);
                    expect(headB).to.equal(expectedB[index]);
