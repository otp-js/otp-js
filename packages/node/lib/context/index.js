...
export class Context {
    ...
    constructor(parent, pid) {
        ...
        this.#mb = new Mailbox(this.logger('message-box'));
        this.#links = l.nil;
    ...
    exit(pid, reason) {
        ...
        return this.#node.exec('exit', [this.#pid, pid, reason]);
    }
    ...
    #forward(operation, name = operation) {
        ...
        this[name] = (...args) => {
            try {
                return this.#node.exec(name, args);
            } catch (err) {
    ...
    #forwardWithPid(operation, name = operation) {
        this[name] = (...args) => {
            try {
                return this.#node.exec(operation, [this.self(), ...args]);
            } catch (err) {
                this.#log(
